function [ue_tp,ue_scheduled,ue_prb,ue_rate_update,ue_sinr_avg] = DL_RR_Schedule( FD_S,pl_d,ue_layout,bs_layout,ue_bs,ue_rate_prb,ue_rate_avg,TTI_counter,N_prb,N_block,N_prb_mue_exp,ue_bs_rx_w,N_power)
...
ue_rate_update=ue_rate_avg;
  if strcmp(FD_S,'FD_RR')
    %if (abs_indicator~=1)
   bs_prb=zeros(nbs,N_block*N_prb);

    for ii=1:nbs;
        bs_ind=ii;
        ue_ind=find(ue_bs(:,bs_ind)==1);
        if isempty(ue_ind)
            continue
        end
        % ue priority sequence
           ue_ind=ue_ind(randperm(length(ue_ind)));

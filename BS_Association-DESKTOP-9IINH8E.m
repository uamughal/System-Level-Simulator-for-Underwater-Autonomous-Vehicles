...
% UBS association
N_UE=size(UE_Layout,1);              %size of the 1st coloumn of 
                                     %UE_Layout matrix shows the
                                     %number of UEs
N_BS=size(MBS_layout,1);             %size of the 1st coloumn of 
                                     %UBS_Layout(not MBS) matrix
                                     %shows the number of UBS
                                     
UE_AP=zeros(N_UE,N_BS,'int8');       %int8 Convert to signed 8-bit integer 
% switch_ue=[];
...
% add macrocell UE
if isempty(UE_Layout)         %isempty True for empty array
    ue_layout=MC_UE;
else
    ue_layout=[UE_Layout];
end
